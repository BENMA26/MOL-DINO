...
from model.retrans import RelaTransEncoder,EquivariantLayerNorm
...
class SinusoidalPositionalEmbedding(nn.Module):
    def __init__(self, embedding_dim: int, padding_idx: int = 0, init_size: int = 1024):
        ...
        self.register_buffer(
            "weights",
            self.get_embedding(init_size + padding_idx + 1, embedding_dim, padding_idx),
            persistent=False,
        )

    @staticmethod
    def get_embedding(num_embeddings: int, embedding_dim: int, padding_idx: int = 0) -> torch.Tensor:
        half_dim = embedding_dim // 2
        if half_dim == 0:
            emb = torch.zeros(num_embeddings, embedding_dim)
    ...
    def forward(self, positions: torch.Tensor) -> torch.Tensor:
        ...
        required_size = max_pos + self.padding_idx + 1
        if self.weights.device != positions.device:
            self.weights = self.weights.to(positions.device)
        if required_size > self.weights.size(0):
            self.weights = self.get_embedding(
                required_size, self.embedding_dim, self.padding_idx
            ).to(self.weights.device)
        return self.weights.index_select(0, positions + self.padding_idx)
...
class StructureUnawareDecoder(nn.Module):
    def __init__(self, hidden_dim, n_heads, n_blocks,args):
        ...
        self.embed_positions = SinusoidalPositionalEmbedding(
            ...
            padding_idx=0,
            init_size=getattr(args, "max_seq_len", 256),
        )
    ...
    def forward(self,data, rep, vec_mask, pos_mask=None):
        node_feature = data.x
        ...
        if hasattr(data, "ptr"):
            starts = torch.repeat_interleave(data.ptr[:-1], data.ptr[1:] - data.ptr[:-1]).to(h.device)
            positions = torch.arange(h.size(0), device=h.device) - starts + 1
        else:
            positions = torch.arange(h.size(0), device=h.device) + 1
        h = h + self.embed_positions(positions).to(h.dtype)
...
class AutoEncoder(nn.Module):
    ...
    def forward(self, data):
        ...
        if self.args.pos_mask and self.args.denoising:
            ...
            return pred_coords, pred_noise,vec, rep.detach()
        if not self.args.pos_mask and self.args.denoising:
