...
class RelaTransEncoder(nn.Module):
    def __init__(self, node_dim, edge_dim, hidden_dim, n_heads, n_blocks,prior_model,args):
        super().__init__()
        self.args = args
        self.use_cls_token = getattr(args, "use_cls_token", False)
        self.cls_distance = float(getattr(args, "cls_distance", 1.0))
    ...
    def forward(self, data, node_feature, edge_index, edge_feature, position, pos_mask=None, return_cls=False):
        ...
        if pos_mask is not None:
            ...
            edge_mask = None
            node_batch = data.batch[~pos_mask]
        else:
            edge_mask = None
        ...
        if self.use_cls_token:
            ...
            cls_start = h.size(0)
            cls_indices = torch.arange(num_graphs, device=h.device) + cls_start
            cls_h = self.cls_token.expand(num_graphs, -1).to(h.dtype)
            h = torch.cat([h, cls_h], dim=0)
            cls_vec = vec.new_zeros((num_graphs, 3, vec.size(-1)))
            vec = torch.cat([vec, cls_vec], dim=0)

            node_indices = torch.arange(num_real_nodes, device=h.device)
            cls_for_nodes = cls_indices[node_batch]
            cls_to_node = torch.stack([cls_for_nodes, node_indices], dim=0)
            node_to_cls = torch.stack([node_indices, cls_for_nodes], dim=0)
            cls_self = torch.stack([cls_indices, cls_indices], dim=0)
            cls_edges = torch.cat([cls_to_node, node_to_cls, cls_self], dim=1)

            edge_index = torch.cat([edge_index, cls_edges], dim=1)
            cls_edge_count = cls_edges.size(1)
            cls_distance = distance.new_full((cls_edge_count, 1), self.cls_distance)
            distance = torch.cat([distance, cls_distance], dim=0)
            cls_edge_vec = edge_vec.new_zeros((cls_edge_count, 3))
            edge_vec = torch.cat([edge_vec, cls_edge_vec], dim=0)
            cls_edge_h = self.cls_edge_embedding.expand(cls_edge_count, -1).to(edge_h.dtype)
            edge_h = torch.cat([edge_h, cls_edge_h], dim=0)
            if edge_mask is not None:
                cls_mask = torch.zeros(cls_edge_count, dtype=edge_mask.dtype, device=edge_mask.device)
                edge_mask = torch.cat([edge_mask, cls_mask], dim=0)
            distance_embedding = self.distance_expansion(distance.squeeze(-1), None)
        ...
        if self.args.dataset == 'qm9' or self.args.dataset == 'md17':
            if self.args.dataset_arg == 'dipole_moment':
                ...
                pred = self.output_model.post_reduce(pred)
            if return_cls:
                return pred, vec, cls_rep
            return pred, vec

        if return_cls:
            return h, vec, cls_rep
        return h, vec
